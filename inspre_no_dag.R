...
w <- make_weights(as.matrix(tce_se),max_med_ratio=50)
tce[is.na(tce)] <- 0
res<-inspre::fit_inspre_from_R(as.matrix(tce),w,verbose=2,lambda=seq(lam_min,lam_max,0.001),its=100,solve_its=10,rho=10,cv_folds=5,DAG=F,min_nz=0.001)
saveRDS(res,paste0('/sc/arion/projects/psychgen/causal_networks_redux/dce_res_',lam_min,'_to_',lam_max,'_results_no_DAG.RData'))
